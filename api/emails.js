...
async function loadEmails() {
  ...
  try {
    if (fs.existsSync(filePath)) {
      ...
      try {
        ...
        console.warn('Failed to seed local emails to Firestore:', dbErr)
      }
      return inMemoryList
    }
  } catch (error) {
  ...
  try {
    ...
    console.warn('Failed to seed default email to Firestore:', dbErr)
  }
  return inMemoryList
}
...
async function saveEmails(emails) {
  ...
  try {
    ...
    console.warn('Failed to write allowed-emails.json (likely read-only serverless environment):', error)
  ...
  if (req.method === 'POST' || req.method === 'DELETE') {
    ...
    if (req.method === 'POST') {
      ...
      if (!saveResult.success) {
        return res.status(500).json({ error: 'Database Write Error: Failed to save changes to Firestore database.' })
      }
